...
5. Proposed Scheme：VDT baseline、Accuracy-preserving sidecar、Event Consistency Vector、Weak Mismatch Labels、Attribution Head、Demo architecture
6. Experiment：数据集、复现设置、baseline、主结果、hard negative、消融、案例分析
   ...
   - 已完成 baseline：VDT strict BLIP-2/GaussianBlur，`target_domain=usa_today,washington_post`，bs64，`F1=0.8032`，`Acc=0.8032`，`AUC=0.8028`；bs128 OOM 作为硬件约束记录。
   - 对比重点：VDT 输出二分类；E3-VDT 在 baseline-preserving 模式下分类指标持平，同时输出 mismatch type + conflict fields + event scores + explanation。
